...
/**
 * Returns the classification label using threshold rows fetched from DB.
 * Returns undefined (not null) when no threshold row exists for the combo.
 */
export function getClassificationFromRows(rows, bowType, ageCategory, gender, roundName, score) {
  const gk = GENDER_KEY[gender];
  if (!gk) return undefined;
  const ak = ageKey(ageCategory);
  const row = rows?.find(r =>
    r.bow_type === bowType && r.age_category === ak && r.gender === gk && r.round_name === roundName
  );
  if (!row) return undefined;
  let best = null;
  for (let i = 0; i < row.thresholds.length; i++) {
    if (row.thresholds[i] !== null && score >= row.thresholds[i]) best = LBL[i];
  }
  return best;
}
...
/**
 * Returns info about the next classification level above currentLabel for a
 * given archer profile. Returns null if already at IGMB or no data available.
 * opportunities is sorted easiest (lowest threshold) first.
 */
export function nextTargetInfo(bowType, ageCategory, gender, currentLabel) {
  const bk = BOW_KEY[bowType];
  const gk = GENDER_KEY[gender];
  if (!bk || !gk) return null;
  const currentIdx = currentLabel ? LBL.indexOf(currentLabel) : -1;
  const nextIdx = currentIdx + 1;
  if (nextIdx >= LBL.length) return null;
  const nextLabel = LBL[nextIdx];
  const bowData = CLSF[bk]?.[ageKey(ageCategory)]?.[gk];
  if (!bowData) return null;
  const opportunities = [];
  for (const [round, thresholds] of Object.entries(bowData)) {
    const t = thresholds[nextIdx];
    if (t != null) opportunities.push({ round, threshold: t });
  }
  opportunities.sort((a, b) => a.threshold - b.threshold);
  return opportunities.length ? { label: nextLabel, opportunities } : null;
}
